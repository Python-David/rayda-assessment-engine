...
from app.schemas.external_api_responses import ExternalUserSuccessResponse, ExternalUserData
from app.schemas.webhooks import UserServiceEvent, UserData
from app.utils.logger import get_logger
from app.db.session import SessionLocal
from app.core.enums import UserEventType, UserStatus
...
def sync_user(event: UserServiceEvent, external_response: ExternalUserSuccessResponse):
    ...
    try:
        ...
        if event_type_str not in UserEventType._value2member_map_:
            logger.warning(f"Invalid event type '{event_type_str}' for user {external_id}. Skipping.")
            return
        ...
        if not org:
            logger.warning(f"Organization {org_id} not found for user {external_id}. Skipping sync.")
            return
        ...
        if event_type == UserEventType.created:
            ...
            if not user:
                logger.warning(f"User {external_id} not found on update event. Skipping.")
                return
            ...
            if not user:
                logger.warning(f"User {external_id} not found on delete event. Skipping.")
                return
            ...
            logger.warning(f"Unhandled event type '{event_type}' for user {external_id}. Skipping.")
